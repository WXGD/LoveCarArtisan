...
- (void)serviceModuleLayoutView {
    ...
    /** 门店 */
    self.storeBtn = [[ModuleView alloc] init];
    self.storeBtn.moduleImage.image = [UIImage imageNamed:@"home_page_store"];
    self.storeBtn.moduleLabel.text = @"门店";
    self.storeBtn.moduleBtn.tag = StoreBtnAction;
    [self.twoModuleBackView addSubview:self.storeBtn];
    /** 2分割线 */
...
- (void)layoutSubviews {
    ...
    /** 服务,会员卡,客户,营销,报表,订单，商城, 账户, 门店 */
    [@[self.serviceBtn, self.membershipCardBtn , self.customerBtn , self.marketingBtn , self.reportBtn , self.orderBtn, self.commercialCityBtn, self.accountBtn, self.storeBtn] mas_makeConstraints:^(MASConstraintMaker *make) {
        @strongify(self)
