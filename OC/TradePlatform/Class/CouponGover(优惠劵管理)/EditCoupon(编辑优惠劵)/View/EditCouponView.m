...
@interface EditCouponView ()<GJTextViewDelegate>

/** 背景scrollview */
@property (strong, nonatomic) UIScrollView *editCouponScrollView;
/** 填充scrollview的view */
@property (strong, nonatomic) UIView *editCouponView;
/** 优惠卷描述 */
...
- (void)editCouponLayoutView {
    /** 背景scrollview */
    self.editCouponScrollView = [[UIScrollView alloc] init];
    [self addSubview:self.editCouponScrollView];
    UITapGestureRecognizer *editCouponTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(editCouponTapAction)];
    [self.editCouponScrollView addGestureRecognizer:editCouponTap];
    /** 填充scrollview的view */
    self.editCouponView = [[UIView alloc] init];
    [self.editCouponScrollView addSubview:self.editCouponView];
    /** 优惠卷名称 */
    self.couponNameView = [[CustomCell alloc] init];
    self.couponNameView.lineStyle = FullScreenLayout;
    self.couponNameView.cellStyle = HorizontalLayoutNotMImgAndVImg;
    ...
    self.couponNameView.rightViceLabel.font = FourteenTypeface;
    self.couponNameView.rightViceLabel.textColor = GrayH1;
    self.couponNameView.rightViceLabel.text = @"名称";
    [self.editCouponView addSubview:self.couponNameView];
    /** 优惠卷面值 */
    self.couponPoundView = [[CustomCell alloc] init];
    self.couponPoundView.lineStyle = FullScreenLayout;
    self.couponPoundView.cellStyle = HorizontalLayoutNotMImgAndVImg;
    ...
    self.couponPoundView.rightViceLabel.textColor = RedColor;
    self.couponPoundView.rightViceLabel.font = FourteenTypeface;
    self.couponPoundView.rightViceLabel.text = @"面值";
    [self.editCouponView addSubview:self.couponPoundView];
    /** 优惠卷是否只能领一张 */
    ...
    [self.editCouponView addSubview:self.isOnlyView];
    ...
    [self.editCouponView addSubview:self.couponOpenTimeView];
    /** 优惠卷结束时间 */
    ...
    [self.editCouponView addSubview:self.couponEndTimeView];
    /** 优惠卷发放数量 */
    ...
    [self.editCouponView addSubview:self.couponTotalView];
    ...
    [self.editCouponView addSubview:self.couponExpiryDateView];
    ...
    [self.editCouponView addSubview:self.couponApplyView];
    /** 优惠卷适用服务Label */
    ...
    [self.editCouponView addSubview:self.coupondescView];
    /** 优惠卷描述标题 */
...
- (void)layoutSubviews {
    ...
    @weakify(self)
    /** 背景scrollview */
    [self.editCouponScrollView mas_makeConstraints:^(MASConstraintMaker *make) {
        @strongify(self)
    ...
    /** 填充scrollview的view */
    [self.editCouponView mas_makeConstraints:^(MASConstraintMaker *make) {
        @strongify(self)
        make.top.equalTo(self.editCouponScrollView.mas_top);
        make.left.equalTo(self.editCouponScrollView.mas_left);
        make.bottom.equalTo(self.editCouponScrollView.mas_bottom);
        make.right.equalTo(self.editCouponScrollView.mas_right);
        make.width.equalTo(self.editCouponScrollView.mas_width);
        make.bottom.equalTo(self.coupondescView.mas_bottom);
    }];
    /** 优惠卷名称 */
    [self.couponNameView mas_remakeConstraints:^(MASConstraintMaker *make) {
        @strongify(self)
        make.top.equalTo(self.editCouponView.mas_top);
        make.left.equalTo(self.editCouponView.mas_left);
        make.right.equalTo(self.editCouponView.mas_right);
    ...
    [self.isOnlyView mas_remakeConstraints:^(MASConstraintMaker *make) {
        @strongify(self)
        make.top.equalTo(self.couponPoundView.mas_bottom).offset(10);
        make.left.equalTo(self.editCouponView.mas_left);
        make.right.equalTo(self.editCouponView.mas_right);
        make.height.mas_equalTo(@50);
    }];
