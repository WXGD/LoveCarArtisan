...
- (void)weekRequest {
    ...
    self.dateReportView.orderListTable.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        ...
        [self.dateReportView.detaReportDataSource detaReportDropRequestData:self.dateReportView.orderListTable params:params viewController:self success:^(NSInteger arrayCount, ReportModel *current, ReportModel *last) {
            ...
            // 界面赋值
            [self weekAssignment:current yestoday:last];
            // 判断是否有订单数据
            if (arrayCount == 0) {
                // 判断是否有数据
                if (arrayCount == 0) {
...
#pragma mark - 按钮点击方法
- (void)weekNavLeftBtnAction {
    [self dismissViewControllerAnimated:YES completion:nil];
}
#pragma mark - 界面赋值
- (void)weekAssignment:(ReportModel *)today yestoday:(ReportModel *)yestoday {
    /** 报表头部view */
    /** 报表头部view标题 */
    self.dateReportView.reportHeaderView.pageTitleLabel.text = @"本周";
    /** 客户数量 */
    self.dateReportView.reportHeaderView.customerView.viceLabel.text = [NSString stringWithFormat:@"%ld", (long)yestoday.count];
    self.dateReportView.reportHeaderView.customerView.mainLabel.text = @"上周新增：";
    /** 交易额 */
    self.dateReportView.reportHeaderView.turnoverByView.viceLabel.text = [NSString stringWithFormat:@"%.2f", yestoday.amount];
    self.dateReportView.reportHeaderView.turnoverByView.mainLabel.text = @"上周交易额：";
    /** 客户数环比 */
    self.dateReportView.reportHeaderView.customerMomView.viceLabel.text = [NSString stringWithFormat:@"%ld", (long)today.count];
    self.dateReportView.reportHeaderView.customerMomView.mainLabel.text = @"本周新增：";
    /** 交易额环比 */
    self.dateReportView.reportHeaderView.turnoverByMomView.viceLabel.text = [NSString stringWithFormat:@"%.2f", today.amount];
    self.dateReportView.reportHeaderView.turnoverByMomView.mainLabel.text = @"本周交易额：";
    
}
#pragma mark - 布局nav
- (void)weekLayoutNAV {
    // 左边
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"nav_back"] style:UIBarButtonItemStyleDone target:self action:@selector(weekNavLeftBtnAction)];
}
#pragma mark - 布局视图
- (void)weekLayoutView {
    /** 日期报表view */
