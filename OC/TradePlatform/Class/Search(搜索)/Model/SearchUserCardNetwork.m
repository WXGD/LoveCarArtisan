...
@interface SearchUserCardNetwork ()
...
@implementation SearchUserCardNetwork
...
- (void)searchUserCardRefreshRequestData:(UITableView *)tableView params:(NSMutableDictionary *)params success:(void(^)(NSMutableArray *userArray))success {
    ...
    parameters[@"type"] = @"3"; // 查询类型 1-用户信息 2-车辆信息 3-卡信息 ，默认为1
    parameters[@"pageSize"] = @"10"; // 每页显示条数
    parameters[@"start"] = @"0"; // 记录位置
    self.start = 10;
    // 发送请求
    [TPNetRequest GET:URL parameters:parameters ProgressHUD:nil falseDate:@"ServiceUserCard" parentController:nil success:^(id responseObject) {
        PDLog(@"responseObject%@", responseObject);
        PDLog(@"params%@", params);
        if ([[NSString stringWithFormat:@"%@", responseObject[@"code"]] isEqual:@"0"]) {
            // 获取数据模型数组
            NSMutableArray *userArray = [UserMemberCardModel mj_objectArrayWithKeyValuesArray:responseObject[@"data"]];
            // 结束下拉刷新
