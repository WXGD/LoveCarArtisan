...
@interface DangerRecordModel ()

/** 请求接点 */
@property(assign, nonatomic) NSInteger start;

@end
@implementation DangerRecordModel
// 下拉刷新
- (void)dangerRecordRefreshRequestData:(UITableView *)tableView
    ...
    NSString *URL = [NSString
                     stringWithFormat:@"%@?c=%@&a=%@&v=%@", API, @"insurance_order", @"list", APIEdition];
    // 拼接请求参数
     ...
     parentController:viewController
              success:^(id responseObject) {
                  ...
                  if ([[NSString stringWithFormat:@"%@", responseObject[@"code"]]
                       isEqual:@"0"]) {
                      // 获取数据模型数组
                      NSMutableArray *orderArray = [DangerRecordModel
