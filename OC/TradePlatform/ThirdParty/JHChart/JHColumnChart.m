...
//        _maxHeight += 4;
    ...
    for (NSInteger i = 0; i<_valueArr.count; i++) {
        ...
        if (_valueShowInfo && _valueShowInfo.count != 0) {
            strArr = _valueShowInfo[i];
        }else {
            strArr = _valueArr[i];
        }
        for (NSInteger j = 0; j<arr.count; j++) {
            ...
            [UIView animateWithDuration:1 animations:^{
                ...
                if (finished) {
                    ...
                    [self.layerArr addObject:textLayer];
                    NSString *str = [NSString stringWithFormat:@"%@",strArr[j]];
                    ...
                    textLayer.frame = CGRectMake((i * strArr.count)*_columnWidth + i*_typeSpace+_originSize.x + _typeSpace, CGRectGetHeight(self.frame) - height - _originSize.y -3 - size.height, _columnWidth * 2, size.height);
                    ...
                    textLayer.alignmentMode = kCAAlignmentCenter;
                    textLayer.contentsScale = [UIScreen mainScreen].scale;
                    UIColor *textLayerColor = _columnTitleBGcolorsArr[j];
                    textLayer.foregroundColor = textLayerColor.CGColor;
                    
                    [_BGScrollView.layer addSublayer:textLayer];
                 
                    
                    //添加折线图
                    if (i==_valueArr.count - 1&&j == strArr.count-1 && _isShowLineChart) {
